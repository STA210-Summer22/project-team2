...
We are curious about what makes a good glass of Vinho Verde wine? How do
wine experts evaluate the quality of wine? We often read that a wine's
quality is related with its aroma, sweetness, tartness, and mellowness,
but do the wine experts also use this common-sense standard we judging
wine's quality? Scientifically speaking, what are the chemical
components contribute to these characteristics? We predict that chemical
components which contribute to wine's aroma, sweetness, ripeness and
freshness will be influential to wine's quality measure points.
...
In our analysis, we defines "good wine" as wine that receives quality
measure scores higher than 7. This is because, the wine magazine 'Wine
Spectator' used a 100-point scale to measure wine's quality, and in
their system, wine that scores at least '80' is defined as 'Good'. In
our data, the highest score than a wine can get is 9, 9 x 80% = 7.2. So
we choose 7 as our cut-off point.
...
-   Free sulfur dioxide and alcohol have a strongly right-skewed
    distribution.
...
Almost for all variables, except for alcohol, there exists significant
difference between red and white wine. Therefore, we would consider the
add interaction between color and other predictor variables in our
model.
...
free sulfur dioxide, density, pH, and sulphates. After testing on
multicollinearity, we found that the correlation coefficient between
free.sulfur.dioxide and total.sulfur.dioxide is 0.72, which indicates
...
two variables in the model, and we choose free.sulfur.dioxide.
...
decide to remove citric acid, dut to its high p-value, to increase our
model's accuracy.
...
or the best AIC model fit.

At last, we fit the picked model--Best AIC Model, to the test data, to
assess its ability on prediction. The following ROC curve plot shows
that the Best AIC Model is doing pretty well on predicting.
...
-   Negative effects:
    ...
    Volatile Acidity: a kind of acid which makes wine tastes like
    vinegar.

-   Positive effects:

    pH: wine with low pH tastes tart and crisp, and higher pH adds to
    wine's ripeness.

    Sulphates: a food preservative widely used in winemaking which
    protects wine against oxidation, and maintains the flavor and
    freshness of wine.
