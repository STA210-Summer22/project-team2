---
title: "Factors Affecting Wine Quality in Vinho Verde, Portugal"
...
To determine the overall performance of the models we fitted, we decided
to graph an ROC curve. ROC is a probability curve that is used to show
the diagnostic ability of binary classifiers. A ROC curve is constructed
by plotting the true positive rate against the false positive rate. The
true positive rate is the proportion of observations that were correctly
predicted to be positive out of all positive observations. Similarly,
the false positive rate is the proportion of observations that are
incorrectly predicted to be positive out of all negative observations.
For example, in context of wine quality, the true positive rate is the
rate in which wine is correctly identified to be good quality.

The ROC curve shows the trade-off between sensitivity (true positive
rate) and specificity (1 - false positive rate). Classifiers that give
...
We then decided to compare the best AIC model with a logistic model with
interactive terms. An interaction occurs if the relation between one
predictor and the outcome variable depends on the value of another
...
To perform the cross validation, we split the training data into 10
folds. The two models have similar accuracy and the roc-auc for 10
folds, so due to the principles of parsimonious, we prefer the reduce
one.
