...
We split the data into 25% testing set and 75% training set and apply the best AIC model to the training set.
...
$logodds(good~wine) = 406.737 + 0.506 \times fixed~acidity - 3.814 \times volatile~acidity + 0.221 \times residual~sugar - 5.531 \times chlorides + 0.011 \times free.sulfur.dioxide - 0.004 \times total.sulfur.dioxide - 428.064 \times density + 2.703 \times pH + 2.449 \times sulphates + 0.449 \times alcohol - 0.853 \times colorwhite$
...
We compare the best AIC model with another logistic model with interactive
terms. We'll call the best AIC model as the reduced model, and the logistic model with interactive terms as the full model. 
...
$logodds(good~wine) = 313.556 + 0.49 \times fixed~acidity - 3.539 \times volatile~acidity - 0.46 \times citric~acid + 0.282 \times residual~sugar - 7.426 \times chlorides + 0.012 \times free.sulfur.dioxide - 0.015 \times total.sulfur.dioxide - 335.148 \times density + 1.669 \times pH + 4.167 \times sulphates + 0.744 \times alcohol + 297.885 \times colorwhite -0.137 \times citric~acid \times colorwhite - 0.003 \times free sulfur dioxide : colorwhite + 0.014 \times total.sulfur.dioxide:colorwhite - 298.31 \times density:colorwhite + 1.475 \times pH:colorwhite - 2.079 \times sulphates:colorwhite - 0.592 \times alcohol:colorwhite$
...
The two models' ROC curves and ROC-AUC are very close, suggesting that the two models have similar performances.

#### Cross Validation

We also conduct a cross validation to further assess the two models' performances. To perform the cross validation, we split the training data into 10 folds. From the plots we can see that the two models have similar accuracy and the ROC-AUC for all 10 folds.
...
$logodds(good~wine) = 399.637 + 0.452 \times fixed~acidity - 3.873 \times volatile~acidity + 0.212 \times residual~sugar - 8.125 \times chlorides + 0.014 \times free.sulfur.dioxide - 0.004 \times total.sulfur.dioxide - 420.73 \times density + 2.663 \times pH + 2.605 \times sulphates + 0.48 \times alcohol - 0.936 \times colorwhite$
...
density, pH, and sulfates. Variables with biggest coefficients are chlorides, volatile acidity, pH and sulphates. 
It is worth-noticing that although the coefficient of density is very high, wine's density varies between approximate 0.99 to 1, therefore it is impossible for wine' density to jump 1 unit. So we do not include density as a influential factor. 

- Negative effects:

  Chlorides: a major contributor to saltiness.

  Volatile Acidity: a kind of acid which produces vinegar-like taste in wine. 

- Positive effects:

  pH: low pH wines will taste tart and crisp. 

  Sulphates: helps to preserve freshness.
...
Coli, Marina Sonegheti et al. Chloride concentration in red wines: influence of terroir and grape type. Food Science and Technology (Campinas) [online]. 2015, v. 35, n. 1 [Accessed 17 June 2022] , pp. 95-99. Available from: <https://doi.org/10.1590/1678-457X.6493>. ISSN 1678-457X. https://doi.org/10.1590/1678-457X.6493.
