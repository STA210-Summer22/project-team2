...
```{r}
newmodel <- stepAIC(full_fit1_eg, direction="both")
```
...
```{r}
#getting errors
#full_fit1_pred <- predict(full_fit1_aug, wine_test)
#full_fit1_pred <- full_fit1_pred %>%
  #mutate(.pred_class = as.numeric(.pred_class))

#optimal <- optimalCutoff(as.numeric(wine_test$good_wine), full_fit1_pred)[1]

#full_fit1_mis <- misClassError(as.numeric(wine_test$good_wine), full_fit1_pred, threshold = optimal)
#accuracy <- mean(as.numeric(wine_test$good_wine) == as.numeric(full_fit1_pred$.pred_class))
```
